...
generate_split_instructions() {
    ...
    echo ""
    echo "##################################################################################"
    echo "###                  ИНСТРУКЦИИ ПО ПЕРЕДАЧЕ И ЗАПУСКУ ЧАСТЯМИ                  ###"
    echo "##################################################################################"
    echo ""
    echo "Однострочник был разбит на ${total_parts} частей. Файлы частей находятся в каталоге '$BUILD_OUTPUT_DIR/'."
    echo ""
    echo ">>> ВНИМАНИЕ: ПОЛНЫЕ И АКТУАЛЬНЫЕ ИНСТРУКЦИИ НАХОДЯТСЯ В ФАЙЛЕ <<<"
    echo ">>>            docs/bootstrap.md                               <<<"
    echo ""
    echo "--- Краткий план действий ---"
    echo "1. Этап 1: На целевой машине выполните подготовительный блок из 'docs/bootstrap-clean.md'."
    echo "2. Этап 2: Для каждой из ${total_parts} частей, скопируйте и выполните соответствующий блок 'cat << ...' из инструкций."
    echo "3. Этап 3: После передачи всех частей, выполните команду 'guido_run'."
    echo ""
    echo "##################################################################################"
}
