...
EOF
    ...
    log_msg "${P_OK} Интерфейс ${C_CYAN}${lan_iface_val}${C_GREEN} (IP: $lan_ip_val) настроен."
...
EOF
    ...
    reg_sneaky_cmd "set_cfg_val /etc/resolvconf.conf deny_interfaces \"${wan_iface_to_deny_dns_val}\""
...
setup_br_rtr_m1_ospf() {
    ...
    if ! grep -q "^ospfd=yes" /etc/frr/daemons; then echo "ospfd=yes" >> /etc/frr/daemons; fi
...
# --- Мета-комментарий: Конец функций-шагов для BR_RTR - Модуль 1 (Сценарий: default) ---