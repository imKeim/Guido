...
setup_isp_m1_net_ifaces() {
    ...
    local to_hqrtr_iface_val; ask_param "Интерфейс ISP к HQRTR" "$m1_isp_to_hqrtr_iface" "to_hqrtr_iface_val"
    local ip_to_hqrtr_iface_val; ask_val_param "IP-адрес ISP для линка к HQRTR (CIDR)" "$m1_isp_to_hqrtr_ip" "is_ipcidr_valid" "ip_to_hqrtr_iface_val"
    local to_brrtr_iface_val; ask_param "Интерфейс ISP к BRRTR" "$m1_isp_to_brrtr_iface" "to_brrtr_iface_val"
    local ip_to_brrtr_iface_val; ask_val_param "IP-адрес ISP для линка к BRRTR (CIDR)" "$m1_isp_to_brrtr_ip" "is_ipcidr_valid" "ip_to_brrtr_iface_val"
...
EOF
    ...
    mkdir -p "/etc/net/ifaces/${to_hqrtr_iface_val}" && find "/etc/net/ifaces/${to_hqrtr_iface_val}" -mindepth 1 -delete
    if ! { echo 'TYPE=eth' > "/etc/net/ifaces/${to_hqrtr_iface_val}/options" &&
          echo "$ip_to_hqrtr_iface_val" > "/etc/net/ifaces/${to_hqrtr_iface_val}/ipv4address"; }; then
        log_msg "${P_ERROR} Ошибка настройки интерфейса ${C_BOLD_RED}${to_hqrtr_iface_val}${P_ERROR}."; return 1
    fi
    log_msg "${P_OK} Интерфейс ${C_CYAN}${to_hqrtr_iface_val}${C_GREEN} (IP: $ip_to_hqrtr_iface_val) настроен."
    reg_sneaky_cmd "echo 'TYPE=eth' > /etc/net/ifaces/${to_hqrtr_iface_val}/options"
    reg_sneaky_cmd "echo '$ip_to_hqrtr_iface_val' > /etc/net/ifaces/${to_hqrtr_iface_val}/ipv4address"

    mkdir -p "/etc/net/ifaces/${to_brrtr_iface_val}" && find "/etc/net/ifaces/${to_brrtr_iface_val}" -mindepth 1 -delete
    if ! { echo 'TYPE=eth' > "/etc/net/ifaces/${to_brrtr_iface_val}/options" &&
           echo "$ip_to_brrtr_iface_val" > "/etc/net/ifaces/${to_brrtr_iface_val}/ipv4address"; }; then
        log_msg "${P_ERROR} Ошибка настройки интерфейса ${C_BOLD_RED}${to_brrtr_iface_val}${P_ERROR}."; return 1
    fi
    log_msg "${P_OK} Интерфейс ${C_CYAN}${to_brrtr_iface_val}${C_GREEN} (IP: $ip_to_brrtr_iface_val) настроен."
    reg_sneaky_cmd "echo 'TYPE=eth' > /etc/net/ifaces/${to_brrtr_iface_val}/options"
    reg_sneaky_cmd "echo '$ip_to_brrtr_iface_val' > /etc/net/ifaces/${to_brrtr_iface_val}/ipv4address"
