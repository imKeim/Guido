...
setup_hqsrv_m1_ssh_srv() {
    ...
    if ! grep -q "^Port $ssh_port_val" /etc/openssh/sshd_config; then
        echo "Port $ssh_port_val" >> /etc/openssh/sshd_config
    fi
    ...
    if ! grep -q "^MaxAuthTries 2" /etc/openssh/sshd_config; then
    ...
    if ! grep -q "^Banner /etc/openssh/banner" /etc/openssh/sshd_config; then
...
EOF
    ...
    log_msg "${P_OK} Файл resolv.conf для ${C_CYAN}${lan_iface_for_dns_val}${C_GREEN} настроен на использование localhost (127.0.0.1)."
    reg_sneaky_cmd "echo -e 'search ${DOM_NAME}\nnameserver 127.0.0.1' > /etc/net/ifaces/${lan_iface_for_dns_val}/resolv.conf"

    if systemctl enable --now dnsmasq && systemctl restart dnsmasq && systemctl is-active --quiet dnsmasq; then
        log_msg "${P_OK} Служба dnsmasq (DNS-сервер) включена и активна."
