#!/bin/bash
# Файл: fx_lib/default/hqrtr/hqrtr_m2_fx.sh
# Содержит функции-шаги для роли HQRTR, Модуль 2, сценарий "default".
# Этот файл подключается (source) функцией _run_step из menu.sh.

# --- Мета-комментарий: Функции-шаги для HQRTR - Модуль 2 (Сценарий: default) ---

# Функция: setup_hqrtr_m2_ntp_srv
# Назначение: Настраивает HQRTR как NTP-сервер для внутренней сети.
setup_hqrtr_m2_ntp_srv() {
    log_msg "${P_ACTION} Настройка NTP-сервера (chrony) на HQRTR..."
    if ! ensure_pkgs "chronyc" "chrony"; then
        log_msg "${P_ERROR} Пакет chrony не установлен."
        return 1
    fi

    local net_hq_srv_allow_val; net_hq_srv_allow_val=$(get_netaddr "$m1_hqrtr_vlan_srv_ip")
    local net_hq_cli_allow_val; net_hq_cli_allow_val=$(get_netaddr "$m1_hqrtr_vlan_cli_ip")
    
    local vlan_mgmt_id_for_ntp_val; ask_val_param "VLAN ID сети Управления (для NTP allow)" "$m1_hqrtr_vlan_mgmt_id_def" "is_vlan_valid" "vlan_mgmt_id_for_ntp_val"
    local vlan_mgmt_ip_for_ntp_val; ask_val_param "IP и маска VLAN Управления (для NTP allow)" "$m1_hqrtr_vlan_mgmt_ip_def" "is_ipcidr_valid" "vlan_mgmt_ip_for_ntp_val"
    local net_mgmt_allow_val; net_mgmt_allow_val=$(get_netaddr "$vlan_mgmt_ip_for_ntp_val")
    
    local net_br_lan_allow_val; net_br_lan_allow_val=$(get_netaddr "$m1_brrtr_lan_ip")
    local net_gre_allow_val; net_gre_allow_val=$(get_netaddr "$m1_hqrtr_gre_tunnel_ip")

    if ! cat <<EOF > /etc/chrony.conf
# Конфигурация NTP-сервера chrony для HQRTR
local stratum 5
...
# Функция: setup_hqrtr_m2_nginx_reverse_proxy
# Назначение: Настраивает Nginx как обратный прокси для Moodle и MediaWiki.
setup_hqrtr_m2_nginx_reverse_proxy() {
    log_msg "${P_ACTION} Настройка Nginx как обратного прокси на HQRTR..."
    if ! ensure_pkgs "nginx" "nginx"; then
        log_msg "${P_ERROR} Пакет Nginx не установлен."
        return 1
    fi

    local moodle_backend_ip_val; moodle_backend_ip_val=$(get_ip_only "$m1_hqsrv_lan_ip")
    local moodle_backend_port_val="$m2_nginx_moodle_backend_port"
    
    local wiki_backend_ip_val; wiki_backend_ip_val=$(get_ip_only "$m1_brsrv_lan_ip")
    local wiki_backend_port_def_val="$m2_nginx_wiki_backend_port_def"
    local wiki_backend_port_val; ask_val_param "Порт MediaWiki на BRSRV (для Nginx proxy_pass)" "$wiki_backend_port_def_val" "is_port_valid" "wiki_backend_port_val"
...
# Функция: setup_hqrtr_m2_dnat_ssh_to_hqsrv
# Назначение: Настраивает DNAT на HQRTR для проброса SSH-соединений на HQSRV.
setup_hqrtr_m2_dnat_ssh_to_hqsrv() {
    log_msg "${P_ACTION} Настройка DNAT для SSH на HQSRV через HQRTR..."
    if ! ensure_pkgs "iptables" "iptables"; then
        log_msg "${P_ERROR} Пакет iptables не установлен."
        return 1
    fi

    local dnat_ext_port_val; ask_val_param "Внешний порт на HQRTR для DNAT SSH на HQSRV" "$m2_dnat_hqrtr_to_hqsrv_ssh_port_var" "is_port_valid" "dnat_ext_port_val"
    local hqsrv_int_ip_val; hqsrv_int_ip_val=$(get_ip_only "$m1_hqsrv_lan_ip")
    local hqsrv_int_ssh_port_val="$DEF_SSH_PORT"
    
    local dnat_listen_iface_val; ask_param "Интерфейс HQRTR для приема DNAT-трафика (например, WAN или LAN Trunk)" "$m1_hqrtr_wan_iface" "dnat_listen_iface_val"
    ...
    if ! iptables -t nat -C PREROUTING -i "$dnat_listen_iface_val" -p tcp --dport "$dnat_ext_port_val" -j DNAT --to-destination "${hqsrv_int_ip_val}:${hqsrv_int_ssh_port_val}" &>/dev/null; then
        ...
        log_msg "${P_OK} Правило DNAT для SSH на HQSRV (порт ${C_CYAN}$dnat_ext_port_val${C_GREEN} -> ${hqsrv_int_ip_val}:${hqsrv_int_ssh_port_val}) добавлено."
        ...
        log_msg "${P_INFO} Правило DNAT для SSH на HQSRV (порт ${C_CYAN}$dnat_ext_port_val${C_RESET}) уже существует."
    fi
...
# --- Мета-комментарий: Конец функций-шагов для HQRTR - Модуль 2 (Сценарий: default) ---