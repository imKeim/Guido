...
*   **Гибкие способы доставки:**
    ...
    *   Скрипт-установщик `guido_installer.sh`.
    *   "Однострочник" (Oneliner) для запуска через текстовый поток (например, serial-порт), с возможностью сжатия `xz` и разбиения на части.
...
Перед использованием Guido необходимо подготовить "полезную нагрузку" и создать установщик. Все артефакты сборки помещаются в каталог `build_output/`.
...
3.  **Запустите скрипт сборки:**
    ...
    ./build_installer.sh
    ```
    *   По умолчанию используется сжатие `gzip`.
    *   Будут созданы:
        *   `build_output/guido_installer.sh`: Готовый к использованию установщик.
        *   `build_output/guido_installer.oneliner.txt`: Файл с полным однострочником.

4.  **(Опционально) Использование `xz` для лучшего сжатия однострочника:**
    Если на целевой машине доступна утилита `unxz`, можно получить меньший однострочник:
    ```bash
    ./build_installer.sh --compressor xz
    ```
    *   **Важно:** `xz` должен быть установлен на машине сборки, а `unxz` — на целевой.

5.  **(Опционально) Разбиение длинного однострочника на части:**
    Если однострочник слишком велик (например, для serial-порта):
    ```bash
    ./build_installer.sh --compressor xz --split-oneliner 49152 # Части по 48 КБ
    ```
    *   В `build_output/` будут созданы файлы частей (например, `oneliner_part_000.xz.b64part`).
    *   Скрипт выведет подробные инструкции по передаче и сборке частей на целевой машине.
...
### 1. Прямой запуск `guido.sh` (для разработки)
   Скопируйте каталог `guido_payload/` на целевую машину.
   ```bash
   cd /путь/к/guido_payload/
   chmod +x guido.sh
   ./guido.sh [аргументы] # например, --pretty --sneaky
   ```

### 2. Запуск через `guido_installer.sh` (рекомендуемый)
   Скопируйте `build_output/guido_installer.sh` на целевую машину.
   ```bash
   chmod +x guido_installer.sh
   ./guido_installer.sh [аргументы]
   ```
   Установщик распакует полезную нагрузку во временный каталог и запустит основной скрипт.

### 3. Запуск через "Однострочник" (Oneliner)

#### 3.1. Целиком (короткий)
   Скопируйте содержимое файла `build_output/guido_installer.oneliner.txt` (далее `ДЛИННАЯ_BASE64_СТРОКА`).

   *   **Если собирали с `gzip` (по умолчанию):**
       ```bash
       echo "ДЛИННАЯ_BASE64_СТРОКА" | base64 -d | gunzip -c | bash -s -- [аргументы_Guido]
       ```
   *   **Если собирали с `xz`:**
       ```bash
       echo "ДЛИННАЯ_BASE64_СТРОКА" | base64 -d | unxz -c | bash -s -- [аргументы_Guido]
       ```

#### 3.2. По частям (длинный)
   Используйте опцию `--split-oneliner` при сборке. Скрипт `build_installer.sh` выведет **подробные пошаговые инструкции** по передаче и сборке частей. Внимательно следуйте им!
   Общий принцип:
   1.  **ЭТАП 1 (Цель):** Подготовка временного файла.
       ```bash
       ...
       ```
   2.  **ЭТАП 2 (Сборка -> Цель):** Для каждой части:
       *   На машине сборки: `cat build_output/oneliner_part_XXX.b64part` (скопировать вывод).
       *   На целевой машине:
           ```bash
           cat <<'END_OF_GUIDO_PART' >> "$TMP_B64_ASSEMBLY_FILE"
           [ВСТАВИТЬ_СКОПИРОВАННУЮ_ЧАСТЬ_BASE64]
           END_OF_GUIDO_PART
           ```
   3.  **ЭТАП 3 (Цель):** Запуск и очистка.
       ```bash
       # Пример для xz с аргументами
       base64 -d "$TMP_B64_ASSEMBLY_FILE" | unxz -c | bash -s -- --pretty --sneaky ; \
       echo 'Скрипт Guido завершен. Удаляю временный файл...' ; \
       rm -f "$TMP_B64_ASSEMBLY_FILE"; unset TMP_B64_ASSEMBLY_FILE
       ```

Подробности см. в [docs/how_to_run.md](docs/how_to_run.md) и [docs/bootstrap.md](docs/bootstrap.md) (пример для разбитого однострочника).
